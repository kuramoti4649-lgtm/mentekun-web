...
{[["list","📋 リスト"],["stats","📊 統計"]].map(function(x) {
          return (
            <button key={x[0]} onClick={function(){playTap();setTab(x[0]);}}
              style={{flex:1,padding:"8px 0",borderRadius:9,border:"none",cursor:"pointer",fontWeight:600,fontSize:13,background:tab===x[0]?"#1E293B":"transparent",color:tab===x[0]?"#F1F5F9":"#475569"}}>
              {x[1]}
            </button>
          );
        })}
      ...
      {tab === "stats" ? (
        <div style={{padding:"0 16px 100px",overflowY:"auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
            {[["登録件数",items.length,"#4A9EFF"],["要対応",over.length+soon.length,"#EF4444"],["今週期限",soon.length,"#F97316"],["問題なし",ok.length,"#22C55E"]].map(function(x) {
              return (
                <div key={x[0]} style={{background:"#0F172A",border:"1px solid "+x[2]+"30",borderRadius:14,padding:"16px 14px"}}>
                  <div style={{fontSize:11,color:"#475569",marginBottom:6}}>{x[0]}</div>
                  <div style={{display:"flex",alignItems:"baseline",gap:4}}>
                    <span style={{fontSize:32,fontWeight:900,color:x[2]}}>{x[1]}</span>
          ...
          <div style={{fontSize:11,color:"#475569",fontWeight:700,marginBottom:8}}>カテゴリ別</div>
          <div style={R.box}>
            {cats.map(function(x) {
              var c = CATEGORIES[x.key];
              var pct = Math.round(x.count/items.length*100);
              return (
                <div key={x.key} style={{padding:"12px 14px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    <span style={{fontSize:13,color:"#CBD5E1"}}>{c.icon} {c.label}</span>
                    <span style={{fontSize:13,fontWeight:700,color:c.color}}>{x.count}件</span>
                  </div>
        ...
        <div>
          <div style={{display:"flex",gap:6,padding:"0 16px 12px",overflowX:"auto"}}>
            {[["all","すべて","#4A9EFF"]].concat(
              Object.entries(CATEGORIES).filter(function(e){return items.find(function(i){return i.category===e[0];});}).map(function(e){return [e[0],e[1].icon+e[1].label,e[1].color];})
            ).map(function(x) {
              return (
                <button key={x[0]} onClick={function(){playTap();setCat(x[0]);}}
                  style={{borderRadius:20,padding:"6px 12px",fontSize:12,fontWeight:600,cursor:"pointer",background:cat===x[0]?x[2]:"#0F172A",color:cat===x[0]?"#fff":"#64748B",border:"1.5px solid "+(cat===x[0]?x[2]:"#1E293B"),whiteSpace:"nowrap",flexShrink:0}}>
                  {x[1]}
                </button>
              );
            })}
          ...
          <div style={{padding:"0 16px 120px"}}>
            {filtered.length === 0 ? (
              <div style={{textAlign:"center",padding:"60px 20px"}}>
                <div style={{fontSize:48}}>🔧</div>
                <div style={{color:"#475569",marginTop:12,fontSize:14}}>まだ登録がありません</div>
              </div>
            ) : filtered.map(function(item) {
              var days = getDays(item.nextDate);
              var u = urg(days);
              var c = CATEGORIES[item.category] || CATEGORIES.custom;
              return (
                <button key={item.id}
                  style={{background:"#0F172A",borderRadius:14,border:"1px solid #1E293B",borderLeft:"3px solid "+c.color,cursor:"pointer",display:"flex",alignItems:"center",gap:12,padding:"14px",marginBottom:10,width:"100%",textAlign:"left"}}
                  onClick={function(){playTap();setSelId(item.id);setView("detail");}}>
                  <div style={{position:"relative",width:56,height:56,flexShrink:0}}>
                    <Ring days={days} size={56}/>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:11,color:"#475569",marginTop:2}}>{c.icon} {c.label} ・ {fmt(item.nextDate)}</div>
                    {item.note ? <div style={{fontSize:11,color:"#334155",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>📝 {item.note}</div> : null}
                  </div>
                  <div style={{fontSize:11,fontWeight:700,color:u.color,background:u.ring,border:"1px solid "+u.color+"40",borderRadius:20,padding:"3px 10px",whiteSpace:"nowrap",flexShrink:0}}>
                    {u.label}
                  </div>
                </button>
              );
      ...
      <button onClick={function(){playTap();setForm({name:"",category:"car",icon:"🔧",intervalDays:365,nextDate:"",note:""});setShowPre(true);setView("add");}} style={R.fab}>＋</button>
      {toast ? <Toast msg={toast}/> : null}
    </div>
  );
}
